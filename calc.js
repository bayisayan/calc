...
function calculator(string) {
    ...
    if (letter.length>0)
        throw  Error('Не допустимые символы, вы ввели это: '+ letter)
    ...
    if (r==1)
        throw Error('Оба числа должны быть записаны врвбскими или римскими цифрами')
    else if(r==2)
        vars = vars.map(v=>rom2arab(v))
    if (vars.some(v=> v<1 || v>10))
        throw Error('Допустимое значение от1 до 10 включительно');
    let arc = string.match(/[+\-*\/]/)[0]
    let result = Math.floor(eval(vars.join(arc)))
    //console.log(result)
